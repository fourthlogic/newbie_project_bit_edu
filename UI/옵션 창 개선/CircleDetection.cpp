...
void CircleDection::Drawing()
{
    ...
    double adjDist = 4; // 직선과 이 이상 떨어진 경우를 제외
    for (int i = 0; i < this->vCirCenters.size(); i++)
    {
        Point center(this->vCirCenters[i].x, this->vCirCenters[i].y);
        if (adjDist < abs(vEquation[0] * center.x - center.y + vEquation[1]) / sqrt(pow(vEquation[0], 2) + 1))
        {
            vCirCenters.erase(vCirCenters.begin() + i);
        }
    }
    ...
    for (int i = 0; i < this->hCirCenters.size(); i++)
    {
        ...
        if (adjDist < abs(hEquation[0] * center.x - center.y + hEquation[1]) / sqrt(pow(hEquation[0], 2) + 1))
        {
            hCirCenters.erase(hCirCenters.begin() + i);
        }
    }
    vEquation = LSM_Vertical(this->vCirCenters); // 수직 원들의 최소제곱법 직선식
    hEquation = LSM_Horizontal(this->hCirCenters); // 수평 원들의 최소제곱법 직선식
