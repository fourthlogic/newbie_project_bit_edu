	...
	if (nFlags & MK_MBUTTON)
	{
		...
		if (start_pos.x == 0)
		{
			if (m_sPt.x < point.x) // 오른쪽으로 끌었을때
			{
				...
				//이미지 왼쪽으로 나갈때
				if ((z_pos.x * PWidth <= -(m_bgRect.right / 5 * 4)))
				{
					z_pos.x = -(m_bgRect.right / 5 * 4) / PWidth;
				}
			}
			else // 왼쪽으로 끌었을때
			{
				...
				//이미지 오른쪽으로 나갈때
				/*if (((PWidth*((int)zoomWidth+2) - z_pos.x) * PWidth <= (m_bgRect.right / 5 * 1)))
				{
				   z_pos.x = (PWidth * ((int)zoomWidth + 2)) - ((m_bgRect.right / 5 * 1) / PWidth);
				}*/
			}
		}
		...
		if (start_pos.y == 0)
		{
			...
			else // 아래로 내렸을때
			{
				...
				if ((z_pos.y * PHeight <= -(m_bgRect.bottom / 5 * 4)))
				{
					z_pos.y = -(m_bgRect.bottom / 5 * 4) / PHeight;
				}
			}
		}
...
BOOL CImgViewerView::OnMouseWheel(UINT nFlags, short zDelta, CPoint pt)
{
	...
	printf("before_Image_pos.x : %f", before_Image_pos.x);
	printf("before_Image_pos.y : %f", before_Image_pos.y);
	printf("\n");
	printf("before_Pixel_pos.x : %f", before_Pixel_pos.x);
	printf("before_Pixel_pos.y : %f", before_Pixel_pos.y);
	printf("\n");
	...
	printf("m_pos.x : %f ", m_pos.x);
	printf("m_pos.y : %f ", m_pos.y);
	printf("\n");
	...
	printf("m_ePt.x : %f ", m_ePt.x);
	printf("m_ePt.y : %f ", m_ePt.y);
	printf("\n");
	...
	printf("after_Image_pos.x : %f ", after_Image_pos.x);
	printf("after_Image_pos.y : %f ", after_Image_pos.y);
	printf("\n");

	printf("after_Pixel_pos.x : %f ", after_Pixel_pos.x);
	printf("after_Pixel_pos.y : %f ", after_Pixel_pos.y);
	printf("\n");
