...
void main()
{
    time_t start;
    time_t end;
    Mat src = imread("image/a (4).png", IMREAD_GRAYSCALE);
    Mat dst;
    start = clock();
...
void GetCornerPoints(Mat& src, vector<Point>& cornerPts)
{
    ...
    //    if (fabs(contourArea(Mat(approx))) > 10000)  //면적이 일정크기 이상이어야 한다. 
    //{
    //        rc = minAreaRect(contours[i]);
    //        Point2f points[4];
    //        rc.points(points);
    //        for (int i = 0; i < 4; i++)
    //            corners.push_back(points[i]);
    //        for (int i = 0; i < 4; i++) {
    //            putText(src, to_string(i), points[i], 1, 1, Scalar(255, 0, 0));
    //        }
...
#pragma endregion
            ...
            //Mat vote = Mat::zeros(cirRect.rows, cirRect.cols, CV_32F);
            ...
            //int count = 0;
            //for (int i = rc.tl().x; i < rc.tl().x + rc.width; i++) {
            //    for (int j = rc.tl().y; j < rc.tl().y + rc.height; j++) {
            //             if (src.at<uchar>(j,i) == 0)
            //                 continue;
            //        for (int t = 0; t < 360 ; t++)
            //        {
            //            int a = i - 2 * cos(t * PI / 180);
            //            int b = j - 2 * sin(t * PI / 180);
            //            if (a >= 0 && a < src.cols && b >= 0 && b < src.rows)
            //                if (src.at<uchar>(b, a) >200)
            //                    count++;
            //        }
            //        if (count > 360)
            //            cirCenters.push_back(Point(i, j));
            //    }
            //}

#pragma region mat vote
...
#pragma endregion
           //cout << max << endl;
            if(max>=60)
                cirCenters.push_back(Point(x+rc.tl().x, y+rc.tl().y));
        }
    }
}
// 미분 함수(prewitt Edge)
