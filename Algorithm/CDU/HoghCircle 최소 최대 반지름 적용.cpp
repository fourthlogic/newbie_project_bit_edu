...
void main()
{
    time_t start;
    time_t end;
    Mat src = imread("image/a (2).png", IMREAD_GRAYSCALE);
    Mat dst;
    ...
    ContourDetection(src, vCirCenters, hCirCenters, vertexPts, 2, 3, 100); // 원 검출
...
void GetCornerPoints(Mat& src, vector<Point>& cornerPts)
{
    ...
    for (size_t i = 0; i < contours.size(); i++)
    {
        ...
        {
            DouglasPeucker(contours[i], approx, 0, true, false);
            ...
            for (int r = radMin; r <= radMax; r++, rCount++)
            {
                ...
                if (max >= 360/rotate)
                {
                    cirCenters.push_back(Point(idx.x + rc.tl().x, idx.y + rc.tl().y));
                     /*cout << "vote" << endl << "[ ";
                     for (int k = 0; k < cirRect.cols; k++) {
                         for (int q = 0; q < cirRect.rows; q++) {
                             cout << vote[q][k] << " ";
                         }
                         cout << endl;
                     }
                     cout << "]" << endl;
                     circle(src, Point(idx.x + rc.tl().x, idx.y + rc.tl().y), r, Scalar(255));
