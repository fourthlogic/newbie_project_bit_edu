...
Point2d intersection(Point2d p1, double a, double b, double a_) {
	/*double x, y;
	double determinant = a * b - a_ * b;
	double c1 = a * (p1.x) + b * (p1.y);
	double c2 = _a * (p1.x) + b1 * (A.second);
...
void CImgViewerView::OnMouseMove(UINT nFlags, CPoint point)
{
		...
		if (IsContainVector(SelectIndex, GroupList)) {
			...
			else
			{
				...
				if (data[SelectIndex].theta == 0)
				{
					...
					printf("o : %f ", o);
					printf("p : %f \n", p);
					printf("0.x : %f \n", data[SelectIndex].RotatePts[0].x);
					printf("0.y : %f \n", data[SelectIndex].RotatePts[0].y);
					printf("1.x : %f \n", data[SelectIndex].RotatePts[1].x);
					printf("1.y : %f \n", data[SelectIndex].RotatePts[1].y);
					printf("2.x : %f \n", data[SelectIndex].RotatePts[2].x);
					printf("2.y : %f \n", data[SelectIndex].RotatePts[2].y);
					printf("3.x : %f \n", data[SelectIndex].RotatePts[3].x);
					printf("3.y : %f \n", data[SelectIndex].RotatePts[3].y);
				}
				else
				{
					if (EdgeIndex == 0)
	...
	else if (MK_LBUTTON == nFlags && (rotateID == TRUE))
	{
		...
		//Point2d e_center = data[SelectIndex].edge[EdgeIndex][0];

		//double dxd, dxi, dyd, dyi;
		//dxd = modf(e_center.x, &dxi);
		//dyd = modf(e_center.y, &dyi);


		//e_center.x = (PWidth * ((int)dxi - z_pos.x)) + (PWidth * dxd);
		//e_center.y = (PHeight * ((int)dyi - z_pos.y)) + (PHeight * dyd);
		double theta;
		theta = atan2((d_pos.x - e_center.x), (d_pos.y - e_center.y)) - atan2((u_pos.x - e_center.x), (u_pos.y - e_center.y));
		
		//if (data[SelectIndex].theta <= CV_PI) 
		//	theta = atan2((d_pos.x - e_center.x), (d_pos.y - e_center.y)) - atan2((u_pos.x - e_center.x), (u_pos.y - e_center.y));
		//else 
		//	theta = atan2((u_pos.x - e_center.x), (u_pos.y - e_center.y)) - atan2((d_pos.x - e_center.x), (d_pos.y - e_center.y));
		//
...
void CImgViewerView::OnPaint()
{
	...
	for (int i = 0; i < ctrlCopyList.size(); i++) {
		SelectDrawShape(&mdcOffScreen, ctrlCopyList[i]);
	}
		...
		for (int i = 0; i < data.GetSize(); i++) 
		{
			...
			if (data[i].shapeType != DrawMode::DEllipse) {
				if (d_pos.x - u_pos.x > 0) {
					...
					//if (data[i].shapeType == DrawMode::DTriangle)
					//{
					//	double topX = (data[i].pts[0].x + data[i].pts[1].x) / 2;
					//	vector<Point2d> tri;
					//	tri.push_back(Point2d(topX, data[i].pts[0].y));
					//	int test = data[i].pts.size();
					//	//tri.insert(tri.end(), data[i].pts.begin() + 2, data[i].pts.end());
					//	tri.push_back(data[i].pts[2]);
					//	tri.push_back(data[i].pts[3]);
					//	if (overlap(vertices, tri)) {
					//		GroupList.push_back(i);
					//		continue;
					//	}
					//	tri.~vector();
					...
					/*for (int j = 0; j < vertices.size(); j++) {
						if (cn_PnPoly(vertices[j], data[i].pts) > 0) {
							GroupList.push_back(i);
							break;
						}
...
{
	int wideNum = 0;
	/*if (shapeType == DrawMode::DTriangle) {
		double topX = (vertices[0].x + vertices[1].x) / 2;
		vector<Point2d> tri;
		tri.push_back(Point2d(topX, vertices[0].y));
		tri.insert(tri.end(), vertices.begin() + 2, vertices.end());

		for (int i = 0; i < tri.size(); i++) {
			int nextpos = (i + 1 >= tri.size()) ? 0 : i + 1;
			if (tri[i].y <= pos.y) {
				if (tri[nextpos].y > pos.y)
					if (isLeft(tri[i], tri[nextpos], pos) > 0)
						++wideNum;
			}
			else {
				if (tri[nextpos].y <= pos.y)
...
void CImgViewerView::drawShape(CDC* pDC, MyShape& shape)
{
	...
	else if (shapeType == DrawMode::DTriangle)
	{
		...
		pts_0.x = (((PWidth * ((int)dxi - z_pos.x)) + (PWidth * dxd)) + ((PWidth * ((int)uxi - z_pos.x)) + (PWidth * uxd))) / 2;
		pts_0.y = (PHeight * ((int)dyi - z_pos.y)) + (PHeight * dyd);
		pts_1.x = (PWidth * ((int)dxi - z_pos.x)) + (PWidth * dxd);
		pts_1.y = (PHeight * ((int)uyi - z_pos.y)) + (PHeight * uyd);
...
void CImgViewerView::SelectDrawShape(CDC* pDC, MyShape& shape)
{
	...
	for (int i = 0; i < rcCount; i++) {
		if (shape.shapeType == DrawMode::DTriangle) {
			...
			if (i == 2 || i == 3) {
			 ...
			 /*if (i == 0) {
				xd_0 = modf(shape.RotatePts[2].x, &xi_0);
				yd_0 = modf(shape.RotatePts[0].y, &yi_0);
			}
			else if (i == 1) {
				xd_0 = modf(shape.RotatePts[1].x, &xi_0);
				yd_0 = modf(shape.RotatePts[0].y, &yi_0);
			}
			else if (i == 2) {
				xd_0 = modf(shape.RotatePts[1].x, &xi_0);
				yd_0 = modf(shape.RotatePts[1].y, &yi_0);
			}
			else if (i == 3) {
				xd_0 = modf(shape.RotatePts[2].x, &xi_0);
				yd_0 = modf(shape.RotatePts[2].y, &yi_0);
			}*/
		}
		else {
	...
	//{
	//	dxd = modf(Center.x, &dxi);
	//	dyd = modf(Center.y, &dyi);
	//	uxd = modf(Rotate.x, &uxi);
	//	uyd = modf(Rotate.y, &uyi);
	...
	if (penID == TRUE) {
		...
		if (shape.RotatePts.size() > 0)
		{
			if (shape.shapeType == DrawMode::DEllipse)
			{
				xd_0 = modf(shape.RotatePts[0].x, &xi_0);
				yd_0 = modf(shape.RotatePts[0].y, &yi_0);
				xd_1 = modf(shape.RotatePts[2].x, &xi_1);
				yd_1 = modf(shape.RotatePts[2].y, &yi_1);
