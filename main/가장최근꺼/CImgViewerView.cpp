...
void CImgViewerView::drawShape(CDC* pDC, MyShape& shape)
{
	...
	else if (shapeType == DrawMode::DTriangle)
	{
		...
		xd_0 = modf(pt.x, &xi_0);
		yd_0 = modf(pt.y, &yi_0);
		xd_1 = modf(shape.pts[2].x, &xi_1);
		yd_1 = modf(shape.pts[2].y, &yi_1);
		xd_2 = modf(shape.pts[3].x, &xi_2);
		yd_2 = modf(shape.pts[3].y, &yi_2);
...
void CImgViewerView::SelectDrawShape(CDC* pDC, MyShape& shape)
{
	...
	if (rotateID == TRUE) {
		...
		{
			Point2d pt = (shape.RotatePts[0] + shape.RotatePts[1]) / 2;

			xd_0 = modf(pt.x, &xi_0);
			yd_0 = modf(pt.y, &yi_0);
			xd_1 = modf(shape.RotatePts[2].x, &xi_1);
			yd_1 = modf(shape.RotatePts[2].y, &yi_1);
			xd_2 = modf(shape.RotatePts[3].x, &xi_2);
			yd_2 = modf(shape.RotatePts[3].y, &yi_2);
