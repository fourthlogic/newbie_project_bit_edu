...
void CImgViewerView::OnPaint()
{
	...
	if (Save_Shape) {
		for (int i = 0; i < data.GetSize(); i++)
			DrawShape_FileSave(&memDC, data[i]);
		CImage image;
		...
		if (IsContainVector(SelectIndex, GroupList)) {
			for (int j = 0; j < GroupList.size(); j++) {
				...
				// 선
					...
					if (EdgeIndex == 2) {
						if ((pow(data[GroupList[j]].RotatePts[0].x, 2) + pow(data[GroupList[j]].RotatePts[0].y, 2)) < (pow(data[GroupList[j]].RotatePts[1].x, 2) + pow(data[GroupList[j]].RotatePts[1].y, 2))) {
	...
	if (SelectIndex != -1 || !GroupList.empty())
	{
		...
		if (lwDlg.DoModal() == IDOK)
		{
			if (!GroupList.empty()) {
				...
				rollback.push_back(gr);
				for (int i = 0; i < GroupList.size(); i++) {
					if (data[GroupList[i]].shapeType == DrawMode::DPoint)
						continue;
					RollbackInfo info;
					info.idx = GroupList[i];
					info.undoShape = data[GroupList[i]];
				...
				if (data[SelectIndex].shapeType == DrawMode::DPoint)
					return;
				RollbackInfo info;
				info.idx = SelectIndex;
				info.undoShape = data[SelectIndex];
