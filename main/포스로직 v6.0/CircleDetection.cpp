...
bool CircleDection::SelectImage()
{
    ...
    if (dlg.DoModal() == IDOK)
    {
        ...
        this->Original.copyTo(this->src);
        this->Original.copyTo(this->Rotate);
...
void CircleDection::CircleDetection(vector<vector<Point>>& contours, vector<Vec3f>& cirCenters) {
    ...
    for (vector<Point> pts : contours) {
        ...
        if (areaValue < 50 && areaValue > 0) {
            ...
            Mat dstX, dstY, norm, angles;
            Mat norm_threshold;
            GaussianBlur(cirRect, cirRect, Size(3, 3), 1.0);
            Sobel(cirRect, dstX, CV_32F, 1, 0, 3);
            Sobel(cirRect, dstY, CV_32F, 0, 1, 3);
            magnitude(dstX, dstY, norm); // 1차미분
            phase(dstX, dstY, angles, false);
            threshold(norm, norm_threshold, this->BGV, 255, THRESH_TOZERO);
            Point2d a, b, c;
            double cc, aa, bb;

            vector<Point2d> _pts;
            vector<double> edge_angles;
            for (int y = 1; y < norm_threshold.rows - 1; y++)
            {
                for (int x = 1; x < norm_threshold.cols - 1; x++)
                {
                    b = Point2d(x, y);
                    bb = norm_threshold.at<float>(y, x);
                    if (bb != 0)
                    {
                        ...
                        cc = BilinearValue<float>(norm_threshold, c);
                        aa = BilinearValue<float>(norm_threshold, a);
                        double now, next;
                        now = bb - aa; // 2차 미분
                        next = cc - bb;
                        if (now * next < 0) // 부호가 반대일 때
                        {
                            double lambda = abs(now) / (abs(now) + abs(next));
...
bool CircleDection::Rotation()
{
    ...
    if (!GetCornerPoints()) // 3점 좌표 추출
        return false;
    // 회전
    double theta = atan2((this->cornerPts[0].x - this->cornerPts[0].x), (this->src.rows - this->cornerPts[0].y))
...
int CircleDection::BallCounting(vector<Point2d>& shape_pts, bool isCricleShape)
{
    ...
    HoughCircles(norm, this->CirCenters, HOUGH_GRADIENT, 1, 0.1, 20, 10, this->radMin,this->radMax);
